...
func (ls LineSegment) is45() bool {
	return abs(ls.pointB.y-ls.pointA.y) == abs(ls.pointB.x-ls.pointA.x)
}
...
func main() {
	...
	for i := minX; i <= maxX; i++ {
		for j := minY; j <= maxY; j++ {
			covers := 0
			for _, line := range lines {
				if (line.parallelToXOrY() || line.is45()) && line.covers(CoOrdinate{i, j, 0}) {
					covers++
				}
			}
			if covers >= 2 {
